...
var constants = {
    ...
    indexerEndCharacter: "]",
    dotCharacter: ".",
    monadicNullCharacter: "?",
    strictMonadicNullCharacter: "??",
    blockStartCharacter: "{"
};
...
var utils = {
    ...
    memberExpressionEnd: function(state, item) {
        if(validators.isMonadicNull(state)) {
            state.item.monadic = true;
            state.item.monadicStrict = false;
            state.next();
        }

        if(validators.isStrictMonadicNull(state)) {
            state.item.monadic = true;
            state.item.monadicStrict = true;
            state.next();
        }
...
var validators = {
    ...
    isMonadicNull: function(state) {
        var lookahead = state.lookahead();
        return state.token && state.token.type === constants.monadicNullCharacter && lookahead && lookahead.type === constants.dotCharacter;
    },

    isStrictMonadicNull: function(state) {
        var lookahead = state.lookahead();
        return state.token && state.token.type === constants.strictMonadicNullCharacter && lookahead && lookahead.type === constants.dotCharacter;
    },
